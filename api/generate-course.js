  ...
  try {
    ...
    const aiRes = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        input: [
          `위치: ${place} (${lat.toFixed(5)}, ${lng.toFixed(5)})`,
          `목표 거리: ${km.toFixed(1)}km`,
          `날씨/상황: ${weather}`,
          `사용자 선호: ${preference}`,
          `이미 저장한 코스 이름: ${savedCourseNames.length ? savedCourseNames.join(', ') : '없음'}`,
    ...
    if (!aiRes.ok) {
      console.error('OpenAI course generation failed', aiRes.status, data && data.error);
